...
% old ROIs
% ROIIndex = {[1,2],[3,4],[49,50],[51,52],[53,54],[57,58],[59,60],[61,62],[63,64],[1],[1]};
% ROIAtlas = {'fsAnatomyMacro.nii','fsAnatomyMacro.nii','fsAnatomyMacro.nii','fsAnatomyMacro.nii', ...
%     'fsAnatomyMacro.nii','fsAnatomyMacro.nii','fsAnatomyMacro.nii','fsAnatomyMacro.nii','fsAnatomyMacro.nii','BA44.nii','PFt.nii'};
% ROIText = {'Precentral','SuperiorFrontalGyrus','SuperiorOccipitalGyrus','MiddleOccipitalGyrus', ...
%     'InferiorOccipitalGyrus','Postcentral','SuperiorParietalLobe','InferiorParietalLobe','SupraMarginal','BA44','PFt'};
...
for iatlas = [1,3,7,8,9]%1:numel(ROIIndex) %[1,3,7,8,9]
    ...
    for isub = 1:numel(allsub)%[1,3,4,6,7,8,9,10]%1:numel(allsub)
        ...
        if strcmp(calculate,'BP')
            ...
            % normalise power
            tmpPowerM = abs(freqM.fourierspctrm(:,ROIelec,freqPoint,:)).^2;
            tmpPowerS = abs(freqS.fourierspctrm(:,ROIelec,freqPoint,:)).^2;
    ...
    if strcmp(calculate,'BP') | strcmp(calculate,'BPall')
        ...
        tMap = zeros(1,size(allMetricS,2));
        pMap = zeros(1,size(allMetricS,2));
        y2plot = zeros(2,size(allMetricS,2));
        se2plot = zeros(2,size(allMetricS,2));
