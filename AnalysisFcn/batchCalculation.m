function varargout = batchCalculation(calculate,inpara)
...
% visual ROIS
% ROIIndex = {[1:4],[4:8],[9:12],[13:14],[25:26]};
...
for iatlas = [1,3,7] %1:numel(ROIIndex) %[1,3,7,8,9]
    ...
    for isub = 1:numel(allsub)%[1,3,4,6,7,8,9,10]%1:numel(allsub)
        ...
        %% section1-2: calculate ERP (Electrode Level) %%
        %%%%%%%%%%%%%%%%%%%%%%%%
        if strcmp(calculate,'ERPelec')
            ...
            % generate index of Subject Electrode and Trial for LME
            metricM = timelockM.avg(ROIelec,:);
            metricS = timelockS.avg(ROIelec,:);
...
%             continue
        ...
        if strcmp(calculate,'PSDhalfmovie')
            ...
            for im = 1:size(camInfo,1)
                ...
                % extract first half data of each movie
                timeInd = freq.time >= camInfo{im,4} & freq.time <= (camInfo{im,4}+camInfo{im,7}/2);
                tmpTS = allTS(:,:,timeInd);
                tsFirst=cat(3,tsFirst,mean(tmpTS,3,'omitnan'));
                
                % extract first half data of each movie
                timeInd = freq.time >= (camInfo{im,4}+camInfo{im,7}/2) & freq.time <= (camInfo{im,4}+camInfo{im,7});
                tmpTS = allTS(:,:,timeInd);
                tsSecond=cat(3,tsSecond,mean(tmpTS,3,'omitnan'));
                
            end
            
            % generate index of Subject Electrode and Trial for LME
            metricM = mean(tsFirst,3);
            metricS = mean(tsSecond,3);
        ...
        %% section3-5: calculate power spectrum difference between views %%
        %%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if strcmp(calculate,'PACind')
            ...
            allSubInd = unique(a.lmeTBL.Sub);
            elecIndM  = a.lmeTBL.Sub==nominal(isub) & a.lmeTBL.Cond=='1';
            elecIndS  = a.lmeTBL.Sub==nominal(isub) & a.lmeTBL.Cond=='2';
            
            powM = squeeze(nanmean(allMetric(elecIndM,:),1));
            powS = squeeze(nanmean(allMetric(elecIndS,:),1));
            
            betaInd = a.Para.freq >= 20 & a.Para.freq <= 30;
            [betaPeakM]=getpeak(powM(betaInd),a.Para.freq(betaInd),log10(20),log10(30),0);
            [betaPeakS]=getpeak(powS(betaInd),a.Para.freq(betaInd),log10(20),log10(30),0);
            
            gammaInd = a.Para.freq >= 60 & a.Para.freq <= 90;
            [gammaPeakM]=getpeak(powM(gammaInd),a.Para.freq(gammaInd),log10(60),log10(100),0);
            [gammaPeakS]=getpeak(powS(gammaInd),a.Para.freq(gammaInd),log10(60),log10(100),0);
            ...
            for ip = 1:numel(ROIelec)
                ...
                seedTS = squeeze(lowFourier.Value(:,ip,:,:));
                searchTS = squeeze(highFourier.Value(:,ip,:,:));
                cfcdata = zeros(size(camInfo,1),1);
                for ii = 1:size(camInfo,1)
                    % extract data of each movie
                    tmpseedTS = seedTS(camInfo{ii,4}:camInfo{ii,4}+fs*(camInfo{ii,7}-camInfo{ii,5}));
                    tmpsearchTS = searchTS(camInfo{ii,4}:camInfo{ii,4}+fs*(camInfo{ii,7}-camInfo{ii,5}));
                ...
                elecPACS = abs(mean(cfcdata(cell2mat(camInfo(:,1))==1)));
                
                
                % generate null distribution of CFC
                shfPACM = zeros(numShuffle,1);
                shfPACS = zeros(numShuffle,1);
                for ishf = 1:numShuffle
                    ...
                    shfPACM(ishf) = abs(mean(cfcdata(cell2mat(camInfo(:,1))==0)));
    ...
    %% section3-2: calculate power spectrum difference %%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if strcmp(calculate,'PSD') | strcmp(calculate,'PSDmovie') | strcmp(calculate,'PSDhalfmovie') | strcmp(calculate,'PSDviews')
        CondIndexM = ones(size(subIndexM));
        CondIndexS = 2*ones(size(subIndexS));
    ...
    %% section5: calculate band power %%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if strcmp(calculate,'BP') | strcmp(calculate,'BPall')  | strcmp(calculate,'ERPelec')
        CondIndexM = ones(size(subIndexM));
        CondIndexS = 2*ones(size(subIndexS));
        allMetricM = squeeze(allMetricM);
        allMetricS = squeeze(allMetricS);
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if strcmp(calculate,'PAC') | strcmp(calculate,'bestPAC') | strcmp(calculate,'PACmovie')
        CondIndexM = ones(size(subIndexM));
        CondIndexS = 2*ones(size(subIndexS));
    ...
    %% section6-2: calculate PAC (individual frequency)%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if strcmp(calculate,'PACind')
        CondIndexM = ones(size(subIndexM));
        CondIndexS = 2*ones(size(subIndexS));
        ...
        frameDataM = double(allMetricM);
        % skip nan point
        if ~any(frameDataM,'all')
            continue
        end
        frameDataS = double(allMetricS);
        ...
        lmeStruct = fitlme(lmeTBL,'Y~Cond+(1|Sub)+(1|Elec)','fitmethod','reml','DummyVarCoding','effects');
        [~,~,lmeStats] = fixedEffects(lmeStruct);
        tMap = lmeStats.tStat(2);
        pMap = lmeStats.pValue(2);
        [randBeta,~,~] = randomEffects(lmeStruct);
        Z = designMatrix(lmeStruct,'random');
        Ycorr = lmeTBL.Y-Z*randBeta;
        obsVal1 = Ycorr(lmeTBL.Cond=='1');
        obsVal2 = Ycorr(lmeTBL.Cond=='2');
        y2plot = [nanmean(obsVal1);nanmean(obsVal2)];
        se2plot = [nanstd(obsVal1)./sqrt(numel(obsVal1));nanstd(obsVal2)./sqrt(numel(obsVal2))];
        rawVal1 = lmeTBL.Y(lmeTBL.Cond=='1');
        rawVal2 = lmeTBL.Y(lmeTBL.Cond=='2');
        yraw = [nanmean(rawVal1);nanmean(rawVal2)];
        seraw = [nanstd(rawVal1)./sqrt(numel(rawVal1));nanstd(rawVal2)./sqrt(numel(rawVal2))];
