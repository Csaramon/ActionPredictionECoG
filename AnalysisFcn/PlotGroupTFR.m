...
lenths = [];
    for ic = 1:numel(Ls.PixelIdxList)
        [xx,yy] = ind2sub(size(contourlines),Ls.PixelIdxList{ic});
        if Para.timePT(max(yy))-Para.timePT(min(yy)) < 0.1
            contourlines(Ls.PixelIdxList{ic}) = 0;
...
caxis(ca)
xlabel(['Time (s)'])
title([filename(1:ti-1) '↔' filename(ti+1:end-4) ' (Nsub=' num2str(numel(unique(lmeTBL.Sub))) ' Nelec=' num2str(numel(unique(lmeTBL.Elec))) ')'])
