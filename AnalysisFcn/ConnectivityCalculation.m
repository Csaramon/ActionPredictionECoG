...
%                 
            ...
            if strcmp(calculate,'PACregion')
                ...
                pacMethod = 'plv'; % coh,plv,mlv,mi,pac
                timeWin = [0 1];
            ...
            if strcmp(calculate,'PACmovie')
                ...
                for ip = 1:size(pairInd,1) % parfor
...
%                             end
            ...
            %% section5-4: calculate cross regional PAC (movie-wise individual frequency) %%
            %%%%%%%%%%%%%%%%%%%%%%%%
            if strcmp(calculate,'PACmovieind')
                ...
                a = load(['C:\Users\qin2\Documents\ActionPredictionECoG\Results\' ...
                    'PSDmovie\fsAnatomyMacro1&5taperFixwinER' filesep ROIText{iseed}]);
                ...
                a = load(['C:\Users\qin2\Documents\ActionPredictionECoG\Results\' ...
                    'PSDmovie\fsAnatomyMacro1&5taperFixwinER' filesep ROIText{isearch}]);
                ...
                [gammaPeakM]=getpeak(powM(gammaInd),a.Para.freq(gammaInd),log10(60),log10(90),0);
                [gammaPeakS]=getpeak(powS(gammaInd),a.Para.freq(gammaInd),log10(60),log10(90),0);
                ...
                allPACM = zeros(size(pairInd,1),1);
                allPACS = zeros(size(pairInd,1),1);
                lowFourier = parallel.pool.Constant(freqLow.fourierspctrm);
                highFourier = parallel.pool.Constant(freqHigh.fourierspctrm);
...
%                         [r, ~, ~] = glmCFC(angle(tmpseedTS), abs(tmpsearchTS), 10, 'AIC');
                    ...
                    % generate null distribution of CFC
                    shfPACM = zeros(numShuffle,1,1);
                    shfPACS = zeros(numShuffle,1,1);
                    for ishf = 1:numShuffle
...
%                     allPACS(ip)  = (elecPACS-mean(shfPACS,1))./std(shfPACS,0,1);
        ...
        %% section5-2: calculate cross regional PAC (individual frequency)%%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if strcmp(calculate,'PACmovieind')
            CondIndexM = ones(size(subIndexM));
            CondIndexS = 2*ones(size(subIndexS));
            ...
            obsVal1 = Ycorr(lmeTBL.Cond=='1');
            obsVal2 = Ycorr(lmeTBL.Cond=='2');
            y2plot = [mean(obsVal1);mean(obsVal2)];
            se2plot = [std(obsVal1)./sqrt(numel(obsVal1));std(obsVal2)./sqrt(numel(obsVal2))];
            rawVal1 = lmeTBL.Y(lmeTBL.Cond=='1');
            rawVal2 = lmeTBL.Y(lmeTBL.Cond=='2');
            yraw = [mean(rawVal1);mean(rawVal2)];
            seraw = [std(rawVal1)./sqrt(numel(rawVal1));std(rawVal2)./sqrt(numel(rawVal2))];
