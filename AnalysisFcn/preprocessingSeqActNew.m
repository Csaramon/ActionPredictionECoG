...
patientID = 'Patient4';  % 1,2,3,4,6,8,9,11,12,13
...
if strcmpi(rerefMethod,'LAR')
    ...
    rerefMap(badChan,:) = 0;
    SelectedReref % remove shared reference eletrodes
...
elseif strcmpi(rerefMethod,'CAR')
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    switch patientID
        case 'Patient6'
            ...
            traShade = -1/numel(nonDepthIndex)*ones(numel(ftData.label));
    ...
    cfg.montage.tra = eye(numel(ftData.label))+traShade;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%-------- Common Average Reference(CAR) --------%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
    elseif strcmpi(rerefMethod,'Bipolar')
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%%-------- Bipolar Reference(CAR) --------%%%%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        tra = eye(numel(ftData.label));
        for ichan = 1:numel(ftData.label)
            refInd = strcmp(BipolarMat{ichan,2},ftData.label);
            tra(ichan,refInd) = -1;
        end
        tra(badChan,:) = 0;
...
uisave('rerefData',['C:\Users\qin2\Documents\ActionPredictionECoG\Data\' patientID filesep 'Analysis\' patientID rerefMethod '_rerefData.mat']);
...
uisave('trlData',['C:\Users\qin2\Documents\ActionPredictionECoG\Data\' patientID filesep 'Analysis\' patientID rerefMethod '_trlData.mat']);
